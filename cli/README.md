...
**搭配詞、字根、同義詞、中文全部寫好了**；其中 347 張的例句也寫好，裝好就能直接複習。
另外 **432 張的例句欄留白給你自己寫** —— 例句是唯一「自己寫過才記得住」的一項。
用 `ielts complete` 一次寫幾張，卡住時輸入 `?` 會給一句參考（預設不顯示）。
佇列**依 sublist 由高頻排到低頻**。

**例句還沒寫不影響背單字** —— `ielts review` 照樣會出這些卡，
背面有搭配詞、字根、同義詞、中文；寫了例句只是讓卡片更完整。
資料的唯一來源在專案根目錄的 `tools/`，改完跑 `python3 tools/build_seed.py` 重新產生。

**每天最多認識 30 個新字**（`ielts config --new-per-day N`，5–60），到期的舊字不受限制 ——
沒有這個上限，整份 AWL 會在第一天全部到期。
算的單位是「字」不是「軌」：同一個字在 `review` 認過之後，
`spell`、`syn` 再出它不會再佔一次額度。
...
```bash
ielts config                 # 看目前的設定（含今天已認幾個新字、還剩幾個）
ielts config --no-zh         # 關掉中文意思：卡片背面、拼字題目、補完模式都純英文
ielts config --zh            # 再打開
ielts config --new-per-day 30   # 每天最多認識 30 個新字（5–60）
```

設定存在資料庫的 `meta` 表，之後每次都生效。
`--new-per-day` 只擋新字：到期的舊字一律照排程出，不受它影響。
算的單位是「字」不是「軌」，所以設 30 就是一天 30 個新字，不是 30×3 個練習項目。
...
```bash
cd cli
python3 -m unittest discover -s tests -t .   # 165 個測試
```

涵蓋 SM-2 排程數學、挖空與拼字比對（含不規則動詞）、同義詞寬鬆比對、資料層查詢與三軌獨立性、
每日新字上限（含「額度數的是字不是軌」）、中文開關（含關掉之後拼字佇列的變化）、CSV 匯入的各種髒資料、streak 計算、種子資料內容驗證，
