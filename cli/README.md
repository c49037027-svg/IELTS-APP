...
| 分類 | 張數 | 四維度齊全 |
|---|---:|---:|
| AWL 學術詞彙（570 字頭全收，`topic` = Sublist 1–10） | 564 | 564 |
| 高頻話題字（7 個主題 × 20） | 140 | 140 |
| Task 1 圖表用語（依功能分組） | 39 | 39 |
| 口說表達（依功能分組） | 36 | 36 |

**779 張卡的四個維度全部寫齊**，裝好就能直接複習，不用先補卡。
`ielts complete` 仍然留著：你自己匯入不完整的 CSV 時會用到，
補完佇列**依 sublist 由高頻排到低頻**。
...
```bash
cd cli
python3 -m unittest discover -s tests -t .   # 157 個測試
```
